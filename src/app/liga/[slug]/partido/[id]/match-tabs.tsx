 ...
 * "Cronología", "Previa" (probabilidades / bajas / alineaciones / cuotas) y
 * "Cara a cara" (historial). Copiado/adaptado de
 * `mundial/partido/[id]/match-tabs.tsx` (que sigue intacto) — la única
 * diferencia real es que `H2HPanel` recibe `competitionSlug` para construir
 * los enlaces a `/liga/[slug]/partido/[id]` en vez de `/mundial/partido/[id]`
 * (los demás paneles ya eran genéricos: trabajan por fixture id, no por
 * competición). Previa y H2H se cargan bajo demanda al abrir su pestaña.
 */
...
export default function MatchTabs({
  ...
  home: Team;
  away: Team;
  /** Slug de la competición (p. ej. "laliga"), para los enlaces del panel H2H. */
      ...
      <div className="space-y-2">
        {matches.map((m) => {
          ...
          return (
            <Link
              key={m.id}
              href={`/liga/${competitionSlug}/partido/${m.id}`}
              className="panel"
              style={{
                display: "block",
                padding: "12px 14px",
