...
export async function generateMetadata({
  ...
  const p = await getPlayerSeasonStats(Number(id)).catch(() => null);
  if (!p) return { title: "Jugador · Mundial 2026 | Soy Reinaldo" };
  const title = `${p.name} · Mundial 2026`;
  const description = `${p.goals} goles y ${p.assists} asistencias de ${p.name} en el Mundial 2026.`;
  return {
    title,
    description,
...
export default async function JugadorPage({
  ...
  const [p, extras] = await Promise.all([
    getPlayerSeasonStats(pid).catch(() => null),
  ...
  return (
    <div className="panel" style={{ overflow: "hidden", marginBottom: 24 }}>
      <div className="shead" style={{ padding: "12px 16px 0" }}>
        <h2 style={{ fontSize: "1.05rem" }}>Totales del Mundial</h2>
        {p.rating != null && (
          <span
